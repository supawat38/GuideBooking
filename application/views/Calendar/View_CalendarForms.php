...
<div class="col-lg-12">
	<?php if($ActionMode == 1){ ?>
		<div class="row">
			<div class="col-lg-2 col-2">
				<select class="form-control jSelectedsingle" id="AddEditYear">
			...
			<div class="col-lg-3 col-3">
				<button class="btn btn-secondary BTNConfirmRegis" style="width:150px;height:40px;font-size: 20px !important; font-family: 'THSarabunNew';" onclick="CheckAddGuideCalendar()">เพิ่มตารางงาน</button>
			</div>
		</div>
	<?php } else { ?>
		<div class="row">
			<div class="col-lg-12 col-12">
				<img src="<?=base_url()?>/application/assets/images/icon/edit-calendar.png" style="width:16px; margin-right: 10px;"> 
					<label class="labelHead" >เดือน<?=ConvertThaiMonth($AddMonth);?> <?=$AddYear?></label>
			</div>
		</div>
	<?php } ?>
			...
			<tbody>
				<?php 
					...
					<tr>
						...
						<td>
							<?=$showDate?>
							<?php if($calenDayStatus == 1){ ?><label style="color:green; font-weight: bold;">[ <img src="<?=base_url()?>/application/assets/images/icon/user.png" style="width:18px"> ได้รับการจองแล้ว ]<label><?php } ?>
							<input type="hidden" name="CalendarDate[]" value="<?=$calenDate?>">
						</td>
						<td>
							<?php if($calenDayStatus == 1){ ?>
								<select class="form-control" name="CalendarSet[]" style="display:none">
	...
	<div class="row">
		...
		<div class="col-lg-4 col-4 text-right" style="padding-bottom:10px;">
			...
			<button type="button" class="btn btn-primary waves-effect waves-light mybnt BTNConfirmRegis" onclick="SaveGuideCalendar()">บันทึก</button>
		</div>
	</div>
</div>
